...
<div class="tg-bannerholder">
                                    ...
                                    <div class="form-group">
                                        <div class="tg-select">
                                            <select name="tour-category" class="selectpicker" data-live-search="true" data-width="100%">
                                                ...
                                                if (!empty($terms) && !is_wp_error($terms)){ ?>
                                                    <option data-tokens="<?php esc_attr_e('Adventure Type','efora'); ?>"><?php esc_attr_e('Adventure Type','efora'); ?></option>
                                                    <?php foreach($terms as $term){ ?>
                                                        <option data-tokens="<?php echo esc_attr($term->name); ?>"><?php echo esc_attr($term->name); ?></option>
                                                    <?php }
                                                } else{ ?>
    ...
    <?php $background_images_sone = efora_get_field('background_images_sone');
    if(is_array($background_images_sone) && count($background_images_sone) > 0){ ?>
        <div id="tg-homeslider" class="tg-homeslider owl-carousel tg-haslayout">
            <?php foreach($background_images_sone as $img){ ?>
                <figure class="item" data-vide-bg="poster: <?php echo esc_url($img['url']); ?>" data-vide-options="position: 0% 50%"></figure>
            <?php } ?>
        </div>
    <?php } ?>
</div>
