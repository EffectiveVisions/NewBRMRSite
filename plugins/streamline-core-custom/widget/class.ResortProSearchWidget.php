...

{
...

    {
      ...
      $this->criteria = array(
        ...
        'guests_dropdown'  => __( 'Guest Dropdown', 'streamline-core' ),
...

    {
        ...
        for ($i = 0; $i < count($fields); $i++) {
            ...
            if ($field != "viewall_button2") {
                ...
                // adding Class ID based on Class
   ...
   if($currentpage == $homepage or $currentpage == '/index.php') {
    ...
    $Classid = "$classname-block" . "-idclass";

    $result .= "<div class='$bs_class' id='$Classid' >";

    $result .= "<div class='form-group $class_feedback $classname-block' id='$classname-block'>";

  } else {

        $Classid = "$classname-block" . "-idclass-homeless";

        $result .= "<div class='$bs_class' id='$Classid' >";

        $result .= "<div class='form-group $class_feedback $classname-block' id='$classname-block-not'>";
                ...
                // add submit_button below location_resort and above amenities per client

                if (!is_front_page()) {

                  if ($field == "location_resort") {
...

    {
        ...
        if (strpos($template, "%checkin_field%") !== false) {
            ...
            $replacements['checkin_field'] = '<input id="search_start_date_single" '.$min_stay.' '.$checkin_days.' data-bindpicker="#search_end_date_single" class="form-control datepicker-single d-block d-sm-none" type="text" ng-model="search.start_date" name="start_date" readonly="readonly" placeholder="'.$instance["booking_dates-label_checkin"].'" /><input id="search_start_date" '.$min_stay.' '.$checkin_days.' data-bindpicker="#search_end_date" class="form-control datepicker d-none d-sm-block" type="text" ng-model="search.start_date" name="start_date" readonly="readonly" placeholder="'.$instance["booking_dates-label_checkin"].'" /><span class="glyphicon glyphicon-calendar form-control-feedback start-date-widget test"></span>';
            ...
            $replacements['checkout_field'] = '<input id="search_end_date_single" class="form-control datepicker-single d-block d-sm-none" type="text" ng-model="search.end_date" name="end_date" readonly="readonly" placeholder="'.$instance["booking_dates-label_checkout"].'" /><input id="search_end_date" class="form-control datepicker d-none d-sm-block" type="text" ng-model="search.end_date" name="end_date" readonly="readonly" placeholder="'.$instance["booking_dates-label_checkout"].'" /><span class="glyphicon glyphicon-calendar form-control-feedback end-date-widget"></span>';
        ...
        if (strpos($template, "%guests_adults_field%") !== false) {
            ...
            $label = 'Guests'; //$instance['guests_adults-label'];
            ...
            if(!empty($label))
             ...
             $replacements['guests_adults_field'] = ResortPro::plusMinus("adult","resortpro_sw_adults", ResortPro::range(0, $max_value), $this->is_var_set('oc', 'intval'), $zero_option, "search.occupants", $zero_option);

        }

        if(strpos($template, "%guests_dropdown_field%") !== false) {
            ...
            $replacements['guests_dropdown_field'] = ResortPro::plusMinusDropDown($max_adults, $label, $label_adults, $max_children, $label_children, $max_pets, $label_pets, $max_beds, $label_beds, $children, $pets, $bedsTrue);
        }
        ...
        if (strpos($template, "%guests_children_field%") !== false) {
            ...
            if(!empty($label))
             ...
             $replacements['guests_children_field'] = ResortPro::plusMinus("child","resortpro_sw_children", ResortPro::range(0, $max_value), $this->is_var_set('ch', 'intval'), $zero_option, "search.occupants_small", $zero_option);
        ...
        if (strpos($template, "%guests_pets_field%") !== false) {
           ...
           $replacements['guests_pets_field'] =  ResortPro::plusMinus("pet","resortpro_sw_pets", ResortPro::range(0, $max_value), $this->is_var_set('pets', 'intval'), $zero_option, "search.pets", $zero_option);

        }

        if (strpos($template, "%number_bedrooms_field%") !== false) {

            $min_value = $instance['number_bedrooms-min'];

            $max_value = $instance['number_bedrooms-max'];



            $label = 'Any # of Bedrooms'; //$instance['number_bedrooms-label'];
            ...
            $replacements['number_bedrooms_field'] = ResortPro::plusMinus("bed","resortpro_sw_bedrooms_number", ResortPro::range($min_value, $max_value), $this->is_var_set('beds', 'intval'), $zero_option, "search.num_bedrooms", $instance['number_bedrooms-plus']);

            //$replacements['number_bedrooms_field'] = ResortPro::dropdown("resortpro_sw_bedrooms_number", ResortPro::range($min_value, $max_value), $this->is_var_set('beds', 'intval'), $zero_option, null, $instance['number_bedrooms-plus'] /*array("-1", $this->_label($instance['number_bedrooms-label'], "Select Bedrooms"))*/);
        ...
        if (strpos($template, "%bedroom_type_field%") !== false) {
            ...
            $beds = StreamlineCore_Wrapper::get_bedroom_types($instance);
            ...
            ?>
                ...
                <script type="text/javascript">
                    ...
                    jQuery(document).ready(function() {
                        ...
                        jQuery('#resortpro_sw_bed').addClass("chosen-select-deselect");

                       jQuery('#resortpro_sw_bed').on('change', function() {
        ...
        if (strpos($template, "%area_field%") !== false) {
            ...
            $replacements['area_field'] = ResortPro::autocomplete($options);
        ...
        if (strpos($template, "%location_resort_field%") !== false) {
            ...
            $label = 'Any Location'; //$instance['location_resort-label'];
...

        {
            ...
            foreach ($a_groups as $group) {
                ...
                $each_am = explode(',', $_GET['amenities']);
                ...
                foreach ($amenities as $amenity) {
                    ...
                    if ($group == $amenity['group_name']) {
                        ...
                        if(in_array($amenity['id'],$each_am)){

                            $html .= '<span class="amenity_item"><input type="checkbox" checked name="resortpro_sw_amenities[]" value="'.$amenity['id'].'"  /><label>' . $amenity['name']. '</label></span>';

                        }else{

                            $html .= '<span class="amenity_item"><input type="checkbox" name="resortpro_sw_amenities[]" value="'.$amenity['id'].'"  /><label>' . $amenity['name']. '</label></span>';
        ...
        $resetsearchid = 'id="resetsearchid"';

        $chooseaminityid = 'id="chooseaminityid"';

        $resetbuttonstyle = 'style="display: inline;"';
        ...
        $replacements['submit_button_field'] = (is_front_page()) ? "<button class=\"{$instance['submit_button-class']}\" type=\"submit\">Search</button>" : "<button class=\"{$instance['submit_button-class']}\" type=\"submit\">{$instance['submit_button-text']}</button>";
...
if( function_exists('kickout') ) kickout( 'instance', $instance );
          ...
          if($currentpage == $homepage or $currentpage == '/index.php') {
            ...
            $replacements['viewall_button_field'] = "<a class=\"{$instance['viewall_button-class']}\" href=\"{$instance['viewall_button-url']}\" . $chooseaminityid .>Choose Amenities </a>";



        } else {

            $replacements['viewall_button_field'] = "<a ng-click=\"clearFilter()\"   class=\"{$instance['viewall_button-class']}\" href='javascript:void(0)'>Reset Search</a>";
