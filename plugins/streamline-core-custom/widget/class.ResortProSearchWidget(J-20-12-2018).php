...
class ResortProSearchWidget extends WP_Widget
{
    ...
    function __construct()
    {
      $this->criteria = array(
        ...
        'guests'          => __( 'All Guest Dropdown', 'streamline-core' ),
    ...
    private function options($field)
    {
        $labels = array(
            ...
            'group_type'      => __( 'Group Type:', 'streamline-core' ),
            'home_type'       => __( 'Home Type:', 'streamline-core' ),
            'guests'          => __( 'All Guests:', 'streamline-core' )
        ...
        switch ($field) {
            ...
            case 'sorting_order':
                ...
                break;
            case 'guests':
                $result['alabel'] = array('text', 'Adults Label', 'Adults');
                $result['clabel'] = array('text', 'Children Label', 'Children');
                $result['bedslabel'] = array('text', 'Bedrooms Label', 'Bedrooms');
                $result['plabel'] = array('text', 'Pets Label', 'Pets');
                $result['max'] = array('integer', __( 'Maximum adults', 'streamline-core' ), 10, 100);
                $result['cmax'] = array('integer', __( 'Maximum children', 'streamline-core' ), 0, 100, 0);
                $result['bedsmax'] = array('integer', __( 'Maximum bedrooms', 'streamline-core' ), 0, 100, 0);
                $result['pmax'] = array('integer', __( 'Maximum pets', 'streamline-core' ), 0, 2, 0);
                $result['children'] = array('checkbox', __( 'Children', 'streamline-core' ), 0, 1);
                $result['bedstrue'] = array('checkbox', __( 'Bedrooms', 'streamline-core' ), 0, 1);
                $result['pets'] = array('checkbox', __( 'Pets', 'streamline-core' ), 0, 1);
                break;
        };
        return $result;
    }
        ...
        <div class="resortpro-widget-dialog wp-dialog" id="<?php echo $modal_div_id?>">
          ...
          <div class="resortpro-widget-filters">
            ...
            <ul id="<?php echo $modal_div_id?>-sortable1" class="connectedSortable connectedSortable-left">
               <?php foreach ($this->criteria as $id => $name):
    ...
    private function default_template($instance)
    {
        ...
        for ($i = 0; $i < count($fields); $i++) {
            ...
            if ($field != "viewall_button2") {
                ...
                $result .= "<div class='c-search-widget__item $bs_class'>";
                $result .= "<div class='form-group $classname-block'>";
    ...
    private function parse_template($template, $instance)
    {
        ...
        $fields = explode(",", $instance['fields']);
        foreach ($fields as $field) {
            if (!($field == 'booking_dates' || $field == 'submit_button'))
        ...
        if (strpos($template, "%checkin_field%") !== false) {
			...
			$sd = (isset($_GET['sd'])) ? filter_var($_GET['sd']) : '';
			$ed = (isset($_GET['ed'])) ? filter_var($_GET['ed']) : '';
			$min_stay = '';
			$checkin_days = '';
			...
			$replacements['checkin_field'] = '<div class="c-input-group input-group"><input id="search_start_date" '.$min_stay.' '.$checkin_days.' data-turndates="'.implode(',', $arr_turn_days).'" data-bindpicker="#search_end_date" class="form-control datepicker" type="text" name="start_date" readonly="readonly" placeholder="'.$instance["booking_dates-label_checkin"].'" value="'.$sd.'" /><span class="c-input-group__btn input-group-btn"><a class="btn btn-default btn-calendar-in js-calendarBtn" href="#"><i class="glyphicon glyphicon-calendar"></i></a></span></div>';
			...
			$replacements['checkout_field'] = '<div class="c-input-group input-group"><input id="search_end_date" data-turndates="'.implode(',', $arr_turn_days).'" class="form-control datepicker" type="text" name="end_date" readonly="readonly" placeholder="'.$instance["booking_dates-label_checkout"].'" value="'.$ed.'" /><span class="c-input-group__btn input-group-btn"><a class="btn btn-default btn-calendar-in js-calendarBtn" href="#"><i class="glyphicon glyphicon-calendar"></i></a></span></div>';
        ...
        if (strpos($template, "%bedroom_type_field%") !== false) {
          ...
          $filter_neighborhood_id = StreamlineCore_Settings::get_options( 'property_neighborhood_id' );
          $filter_resort_id = StreamlineCore_Settings::get_options( 'property_resort_area_id' );
          $filter_location_id = StreamlineCore_Settings::get_options( 'property_loc_id' );
          ...
          if(!empty($filter_neighborhood_id)){
            $params['neighborhood_area_id'] = $filter_neighborhood_id;
          }
          if(!empty($filter_resort_id)){
            $params['resort_area_id'] = $filter_resort_id;
          }
          ...
          foreach ($a_groups as $group) {
              ...
              foreach ($amenities as $amenity) {
                  if ($group == $amenity['group_name']) {
                      $html .= '<span class="amenity_item"><input type="checkbox" name="resortpro_sw_amenities[]" value="'.$amenity['id'].'"  /> ' . $amenity['name'] . '</span>';
                  }
              }
              //$html .= '</div>';
          }
        ...
        if (strpos($template, "%guests_field%") !== false) {
            $enable_plus_minus = StreamlineCore_Settings::get_options('enable_plus_minus');
            $enable_plus_minus = ($enable_plus_minus == '1') ? true : false;
            $max_adults = $instance['guests-max'];
            $max_children = $instance['guests-cmax'];
            $max_pets = $instance['guests-pmax'];
            $max_beds = $instance['guests-bedsmax'];

            $label = $instance['guests-label'];
            $label_adults = $instance['guests-alabel'];
            $label_children = $instance['guests-clabel'];
            $label_pets = $instance['guests-plabel'];
            $label_beds = $instance['guests-bedslabel'];

            $children = $instance['guests-children'];
            $pets = $instance['guests-pets'];
            $bedsTrue = $instance['guests-bedstrue'];

            if ($enable_plus_minus){
                $replacements['guests_field'] = ResortPro::plusMinusDropDown($max_adults, $label, $label_adults, $max_children, $label_children, $max_pets, $label_pets, $max_beds, $label_beds, $children, $pets, $bedsTrue);
            }
