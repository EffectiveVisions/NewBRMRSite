...
class StreamlineCore_AJAX{
  ...
  public function __construct() {
    ...
    // get countries
    add_action( 'wp_ajax_streamlinecore-get-countries', array( &$this, 'get_countries' ) );
    add_action( 'wp_ajax_nopriv_streamlinecore-get-countries', array( &$this, 'get_countries' ) );

    // get states
    add_action( 'wp_ajax_streamlinecore-get-states', array( &$this, 'get_states' ) );
    add_action( 'wp_ajax_nopriv_streamlinecore-get-states', array( &$this, 'get_states' ) );

    add_action( 'wp_ajax_streamlinecore-api-request', array( &$this, 'api_request' ) );
    add_action( 'wp_ajax_nopriv_streamlinecore-api-request', array( &$this, 'api_request' ) );

    add_action( 'wp_ajax_streamlinecore-payment-login', array( &$this, 'payment_login' ) );
    add_action( 'wp_ajax_nopriv_streamlinecore-payment-login', array( &$this, 'payment_login' ) );
  }
  ...
  public function api_request(){
    ...
    $params =  stripslashes(urldecode($_REQUEST['params']));
    ...
    if(isset($result['data']['available_properties']['property'])){
      if(isset($result['data']['available_properties']['property']['id'])){
        $tempProperty = $result['data']['available_properties']['property'];
        $result['data']['available_properties']['property'] = array();
        $result['data']['available_properties']['property'][0] = $tempProperty;
      }
      foreach($result['data']['available_properties']['property'] as $key => $property){
        foreach($property as $param => $value){
          if(empty($value)){
            $result['data']['available_properties']['property'][$key][$param] = null;
          }
        }      
      }      
    }    
    if(isset($result['data']['property'])){
      if(isset($result['data']['property']['id'])){
        $tempProperty = $result['data']['property'];
        $result['data']['property'] = array();
        $result['data']['property'][0] = $tempProperty;
      }
      foreach($result['data']['property'] as $key => $property){
        foreach($property as $param => $value){
          if(empty($value)){
            $result['data']['property'][$key][$param] = null;
          }
        }      
      }
    }    
  ...
  public function get_countries(){
    $results = StreamlineCore_Wrapper::get_countries();
  ...
  public function get_states(){
    ...
    $result = StreamlineCore_Wrapper::get_states($country);
    if($result['data']){
      wp_send_json_success($result['data']);
    }
    _json_error('Can\'t retrieve states for ' . $country);
  }

  public function payment_login(){
    $confirmation_id = ( isset( $_REQUEST['confirmation_id'] ) ? $this->_sanitize_and_validate_variable( $_REQUEST['confirmation_id'] ) : FALSE );
    $zipcode = ( isset( $_REQUEST['zip_code'] ) ? $this->_sanitize_and_validate_variable( $_REQUEST['zip_code'] ) : FALSE );
    $nonce = ( isset( $_REQUEST['nonce'] ) ? $this->_sanitize_and_validate_variable( $_REQUEST['nonce'] ) : FALSE );

    if(!wp_verify_nonce( $nonce, 'payment-login' )){
      $this->_json_error();
    }

    $reservation_info = StreamlineCore_Wrapper::get_reservation_info( null, $confirmation_id );

    if($reservation_info['data']['reservation']){
      $zip = (string)$reservation_info['data']['reservation']['zip'];

      if($zip == $zipcode){
        wp_send_json_success( array(
          'hash' => $reservation_info['data']['reservation']['hash'],
          'conformation_id' => $reservation_info['data']['reservation']['confirmation_id'],
          'zip' => $reservation_info['data']['reservation']['zip']
          //'reservation' => $reservation_info['data']
        ));
      }
